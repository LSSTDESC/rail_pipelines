...
class InformPipeline(RailPipeline):
    ...
    def __init__(self):
        ...
        self.inform_simplesom = MiniSOMInformer.build(
            model=os.path.join(namer.get_data_dir(DataType.model, ModelType.estimator), "model_simplesom.hdf5"),
             hdf5_groupname='',
        )

        self.inform_somoclu = SOMocluInformer.build(
            model=os.path.join(namer.get_data_dir(DataType.model, ModelType.estimator), "model_somoclu.hdf5"),
            hdf5_groupname='',
        )
